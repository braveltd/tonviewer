 ...
 */
function parseFriendlyAddress (addressString: string) {
  ...
  let workchain: Nullable<number> = null;
  if (addr[1] === 0xff) {
...
export class Address {
  ...
  toString (isUserFriendly?: boolean, isUrlSafe?: boolean, isBounceable?: boolean, isTestOnly?: boolean) {
    ...
    if (!isUserFriendly) {
      ...
      let tag = isBounceable ? bounceableTag : nonBounceableTag;
      if (isTestOnly) {
        tag |= testFlag;
      }
